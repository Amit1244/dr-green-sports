...
export default function SingleAccordion(props) {
    ...
    return (
        <div className="each-accordion w-full text-left mb-4 border-2 border-primary rounded-[2px] bg-black/75">
            <button
                className="w-full text-left text-lg font-normal text-[18px] font-Glancyr py-6 px-6 sm:px-12 flex justify-between items-center gap-4 rounded-[40px]"
                onClick={(e) => {
            ...
            <div className="accordion-container w-full h-[0px] overflow-hidden duration-500 ease-in-out">
                <div className="accordion-text pb-6 px-6 sm:px-12 rounded-[40px]">
                    <p className="font-[300] font-Glancyr ">{props.answer}</p>
                </div>
            </div>
        </div>
    );
